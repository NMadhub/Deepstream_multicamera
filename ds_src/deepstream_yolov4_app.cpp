...
namespace YOLOv4 {
  ...
  void Odin::setPaths(guint batch_size) {
    ...
    str(boost::format("models/YOLOv4/model_b%d_gpu1_fp32.engine") % batch_size );
...
int main(int argc, char *argv[]) {
  ...
  GstElement *pipeline = NULL, *streammux = NULL, *sink = NULL,
             ...
             *nvvidconv = NULL, *nvosd = NULL, *queue1 = NULL, *nvvidconv2 = NULL, *encoder = NULL, *codeparser = NULL, *container = NULL, *filesink = NULL, *tiler;
  GstElement *queue2, *queue3, *queue4, *queue5, *queue6, *queue7, *queue8, *queue9, *queue10;
  ...
  GstBus *bus = NULL;
  char arr[] = "/workspace/ninadm/my_codes/";
  char *output_path = arr;
  ...
  encoder = gst_element_factory_make("avenc_mpeg4", "mpeg4-encoder");
  codeparser = gst_element_factory_make("mpeg4videoparse", "mpeg4-parser");
  container = gst_element_factory_make("matroskamux", "qtmux");
  filesink = gst_element_factory_make("filesink", "filesink");
  strcat(output_path,"test_jul1.mp4");
  ...
  /* Set up the pipeline */
    ...
    if (!odin.display_off) {
      ...
      gst_bin_add_many(GST_BIN(pipeline), queue1,
                      pgie_yolo_detector, queue2, nvtracker, queue3, tiler, queue4, nvvidconv, queue5, nvosd, queue6, nvvidconv2, encoder, codeparser, container, sink, NULL);

      if (!gst_element_link_many(streammux, pgie_yolo_detector, nvtracker,
                                 tiler, nvvidconv, nvosd, nvvidconv2, encoder, codeparser, container, sink, NULL)) {
        g_printerr("Elements could not be linked. Exiting.\n");
        return -1;
      }
    }
  ...
  #else
    gst_bin_add_many(GST_BIN(pipeline), queue1,
                    pgie_yolo_detector, queue2, nvtracker, queue3, tiler, queue4, nvvidconv, queue5, nvosd, queue6, nvvidconv2, queue7, encoder, queue8, codeparser, queue9, container, queue10, filesink, NULL);

    if (!gst_element_link_many(streammux, pgie_yolo_detector, nvtracker,
                               tiler, nvvidconv, nvosd, nvvidconv2, encoder, codeparser, container, filesink, NULL)) {
      g_printerr("Elements could not be linked. Exiting.\n");
      return -1;
    }
  #endif
